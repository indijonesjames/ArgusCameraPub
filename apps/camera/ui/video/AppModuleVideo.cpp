...
/* static */ bool AppModuleVideo::video(void *userPtr, const char *optArg)
{
    if (!VideoPipeline::isSupported())
    {
        ...
        fprintf(stderr, "------------------------------------------------------\n");
        ORIGINATE_ERROR("Unsupported function on Android");
    }
...
bool AppModuleVideo::initialize(Options &options)
{
    ...
    PROPAGATE_ERROR(options.addOption(
        Options::Option("video", 'v', "DURATION", Options::Option::TYPE_ACTION,
    ...
    PROPAGATE_ERROR(options.addOption(
        createValueOption("videoformat", 0, "FORMAT",
            ...
            "available formats.", Dispatcher::getInstance().m_videoFormat)));
    PROPAGATE_ERROR(options.addOption(
        createValueOption("videofiletype", 0, "TYPE",
            "set the video file type. For video format 'h265/vp9' set the file type as 'mkv' "
            "since 'h265 & vp9' are only supported by the 'mkv' container.",
             Dispatcher::getInstance().m_videoFileType)));
